...
func (bq *BuddyQueue) AddRecentProfiles(uid ...string) {
	...
	if profilesLen >= latest.RecentMetCountMax {
		bq.RecentMet = profiles[profilesLen-latest.RecentMetCountMax:]
	} else {
		bq.RecentMet = bq.removeDuplicatesInOrder(append(profiles, bq.RecentMet...))
		if len(bq.RecentMet) > latest.RecentMetCountMax {
			bq.RecentMet = bq.RecentMet[:latest.RecentMetCountMax]
		}
	}
}
